...
impl Generator {
    ...
    unsafe fn generate_enum(&mut self, enumeration: *mut UEnum) -> Result<(), Error> {
        ...
        let mut buffer = BufWriter::with_writer(&mut self.get_package(object)?.file);
        ...
        for variant in rest.iter() {
            write_enum_variant(&mut buffer, enum_name, variant)?;
        }

        if !is_last_variant_autogenerated_max {
            write_enum_variant(&mut buffer, enum_name, last)?;
        }
