...
fn main() {
    ...
    let sdk_path = sdk_path
        .to_str()
        .expect("failed to convert sdk_path to UTF-8");

    // The SDK generator will read a file at this path to learn about where to output the generated SDK.
    let mut file_to_place_sdk_path_in =
        PathBuf::from(env::var_os("OUT_DIR").expect("failed to get output directory"));
    file_to_place_sdk_path_in.push("sdk_path");

    fs::write(file_to_place_sdk_path_in, sdk_path)
        .expect("failed to write sdk_path to output directory");
}
